import CodeMirror from "@uiw/react-codemirror";
import { javascript } from "@codemirror/lang-javascript";
import { oneDark } from "@codemirror/theme-one-dark";
import { useState, useEffect } from "react";
import Actions from "../Action";

const CodeEditor = ({ code, setCode, onRunTrigger, socketRef, roomID }) => {
  const [localCode, setLocalCode] = useState(code || "// Write your code here");

  // Sync local state when external code changes
  useEffect(() => {
    setLocalCode(code);
  }, [code]);

  // Emit code changes to other users
  const handleChange = (value) => {
    setLocalCode(value);
    setCode(value);
    if (socketRef?.current) {
      socketRef.current.emit(Actions.CODE_CHANGE, { roomID, code: value });
    }
  };

  // Run trigger handler
  useEffect(() => {
    if (onRunTrigger) onRunTrigger(localCode);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [onRunTrigger]);

  // Listen for code updates from server
  useEffect(() => {
    if (!socketRef?.current) return;

    const handleCodeChange = ({ code: incomingCode }) => {
      // Prevent redundant re-renders
      if (incomingCode !== null && incomingCode !== localCode) {
        setLocalCode(incomingCode);
        setCode(incomingCode);
      }
    };

    const handleSyncCode = ({ code: syncedCode }) => {
      if (syncedCode !== null && syncedCode !== localCode) {
        setLocalCode(syncedCode);
        setCode(syncedCode);
      }
    };

    socketRef.current.on(Actions.CODE_CHANGE, handleCodeChange);
    socketRef.current.on(Actions.SYNC_CODE, handleSyncCode);

    return () => {
      socketRef.current.off(Actions.CODE_CHANGE, handleCodeChange);
      socketRef.current.off(Actions.SYNC_CODE, handleSyncCode);
    };
  }, [socketRef, localCode, setCode]);

  return (
    <div className="overflow-hidden bg-[#0d1117] relative">
      <div className="absolute top-0 left-0 w-full h-8 bg-[#161b22] border-b border-gray-700 flex items-center px-4 text-xs text-gray-400">
        <span className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-red-500"></span>
          <span className="w-3 h-3 rounded-full bg-yellow-500"></span>
          <span className="w-3 h-3 rounded-full bg-green-500"></span>
        </span>
      </div>

      <div className="h-full pt-8 overflow-auto">
        <CodeMirror
          value={localCode} 
          height="100%"
          theme={oneDark}
          extensions={[javascript({ jsx: true })]}
          onChange={handleChange}
          className="text-base font-mono h-full"
        />
      </div>
    </div>
  );
};

export default CodeEditor;
